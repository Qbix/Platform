...
EOT;
		...
		&$table_comment = '')
	{
		...
		$table_indexes = $this->_introspectTableIndexes($table_name);
		...
		// Calculate indexes
		$indexes_exported = var_export($table_indexes, true);
		$indexes_json     = json_encode($table_indexes);
 ...
 * @return {Boolean}
 */
Base.hasIndexOn = function (columns) {
	var indexes = Base.indexes(), name, idx, i;
	for (name in indexes) {
		idx = indexes[name];
		if (!idx.columns || idx.columns.length < columns.length) {
			continue;
		}
