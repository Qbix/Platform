	...
	static function largestSize($sizes, $useHeight = false, $options = array())
	{
		if (is_string($sizes)) {
			...
			if (!$sizes) {
				...
				throw new Exception("Q_Image::largestSize: Sizes for key '$key' not found in Q_Image::getSizes()");
			}
		}
		$wMax = 0;
		$hMax = 0;
		...
		if ($m) {
			$mw = intval($m[0]);
			$mh = isset($m[1]) ? intval($m[1]) : $mw; // "100" means 100x100
		}
	...
	*/
	static function save($params, $throwIfNotAuthorized = false)
	{
		...
		foreach ($sizes as $size => $name) {
			...
			// sorts it first, so the square file it could link to is never one this
			// call wrote -- only one left by an EARLIER upload to the same path, and
			// linking to that would publish the previous image.
